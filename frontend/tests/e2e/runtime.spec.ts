...
test.describe('Runtime Diagnostics', () => {
  test('should load the page and log console/network activity', async ({ page }) => {
    ...
    const networkRequests: { url: string; status: number; statusText: string; responseBody?: string }[] = [];
    ...
    await page.screenshot({ path: 'playwright-report/runtime-screenshot.png', fullPage: true });
    ...
    networkRequests.forEach(req => {
      ...
      if (req.responseBody) {
        const preview = req.responseBody.length > 200
